...
const unsigned long minweight_primpoly[]=
//
...
// Also taken from the fxt book, provide an own implementation
inline uint64_t gf2n_mult(uint64_t a, uint64_t b,
...
//   bitpolmod_times_x(a, c, h)
{
    ...
    do
    {
        { if (b&1) t^=a;  b>>=1; uint64_t s=a&h; a<<=1;  if (s) a^=c; }
        { if (b&1) t^=a;  b>>=1; uint64_t s=a&h; a<<=1;  if (s) a^=c; }
        { if (b&1) t^=a;  b>>=1; uint64_t s=a&h; a<<=1;  if (s) a^=c; }
        { if (b&1) t^=a;  b>>=1; uint64_t s=a&h; a<<=1;  if (s) a^=c; }
    }
...
// Addition/subtraction in GF(2^n) is a simple XOR operation
// independent of the modulus polynomial
inline uint64_t gf2n_add(uint64_t a, uint64_t b) {
    return a ^ b;
}
